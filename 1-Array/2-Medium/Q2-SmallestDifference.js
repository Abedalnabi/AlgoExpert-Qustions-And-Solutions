...
Sample Input
  ...
  arrayTwo = [26, 134, 135, 15, 17]
    ...
    arrayOne = [-1, 0,,3 10, 20, 28]
    arrayTwo = [1,17,26, 134, 135]
  ...
  Hint 3
  Start by sorting both arrays, as per Hint #2. Put a pointer at the beginning of both arrays and evaluate the absolute difference 
  of the pointer-numbers. If the difference is equal to zero, then you've found the closest pair; otherwise, increment the pointer of 
  the smaller of the two numbers to find a potentially better pair.
...
function smallestDifference(arrayOne, arrayTwo) {
	...
	let currentDifferences = Infinity;
	let smallestPair = [];
	while (idxOne < arrayOne.length && idxTwo < arrayTwo.length) {
		let firstNum = arrayOne[idxOne];
		let secondNum = arrayTwo[idxTwo];
		if (firstNum < secondNum) {
			currentDifferences = secondNum - firstNum;
			idxOne++;
		} else if (secondNum < firstNum) {
			currentDifferences = firstNum - secondNum;
			idxTwo++;
		} else {
			return [firstNum, secondNum];
		}
		if (smallestDifference > currentDifferences) {
			smallestDifference = currentDifferences;
